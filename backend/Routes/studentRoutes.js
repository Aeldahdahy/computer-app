        ...
        try {
            if (student_id && student_id !== '%' || email) {
                ...
                const [studentData] = await connection.query(
                    'SELECT student_id, full_name, email, dob, phone_number, department_id, status FROM student WHERE student_id = ? OR email = ?',
                    [student_id, email]
                );
                ...
                const [studentData] = await connection.query(
                    'SELECT student_id, full_name, email, dob, phone_number, department_id, status FROM student'
                );
    ...
    try {
        ...
        try {
            ...
            const [studentData] = await connection.query(
                ...
                [uniqueID, fname, lname, email, hashedPassword, dob, phone_number, department_id, status]
            );
